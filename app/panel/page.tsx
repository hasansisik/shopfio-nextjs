...
export default function PanelPage() {
  ...
  const activeApp = applications[2] || applications[0]
  
  const stats = [
    { label: "Aktif Süreç", value: applications.length, icon: Activity, color: "text-[#95BF47]", bg: "bg-[#95BF47]/10" },
    { label: "Bekleyen Aksiyon", value: "1", icon: Zap, color: "text-orange-500", bg: "bg-orange-50" },
    ...
    <div className="flex-1 p-6 md:p-10 space-y-10 min-h-screen max-w-[1400px] mx-auto pb-24">
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-3">
          ...
          >
            Tekrar Hoş Geldin, <span className="text-[#95BF47]">Hasan</span> 👋
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
           ...
           <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-12">
              <div className="flex items-center gap-6 shrink-0">
                 ...
                 <div className="space-y-2">
                    ...
                    <h3 className="text-2xl font-black text-gray-900 tracking-tight">{activeApp.service}</h3>
                    <p className="text-xs text-gray-400 font-medium flex items-center gap-2">
                       ID: <span className="text-gray-900 font-black">#{activeApp.id}</span>
                       <span className="w-1 h-1 rounded-full bg-gray-300" />
                       Son Güncelleme: <span className="text-gray-900 font-bold">{activeApp.date}</span>
                    </p>
                 </div>
              </div>
              ...
              <Link href={`/panel/basvurular/${activeApp.id}`} className="shrink-0 w-full lg:w-auto">
                  ...
                  <div className="flex-1 space-y-6">
                     <div className="space-y-2">
                        ...
                        <div className="flex items-baseline gap-1">
                           <span className="text-3xl md:text-4xl font-black tracking-tighter">{tier.price}</span>
                           <span className={cn("text-[10px] font-bold", tier.highlight ? "text-white/60" : "text-gray-400")}>'den başlayan</span>
                        </div>
                     </div>
